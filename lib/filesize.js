...
module.exports = {
  format: function(value, decimal, options) {
    var name, power, ref, stop, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3;
    if (decimal == null) {
      decimal = 0;
    }
    if (options == null) {
    ...
    if (options.bit || ((_ref = options.to) != null ? _ref[1] : void 0) === 'b') {
      value *= 8;
    }
    if (options.from) {
    ...
    _ref2 = ['', 'K', 'M', 'G', 'T', 'P', 'E', 'Z', 'Y'];
    for (power = _j = 0, _len1 = _ref2.length; _j < _len1; power = ++_j) {
      name = _ref2[power];
      ref = Math.pow(1024, power + 1);
      name += options.bit || ((_ref3 = options.to) != null ? _ref3[1] : void 0) === 'b' ? 'b' : 'B';
      stop = options.to ? options.to === name : value < ref;
      if (stop) {
    ...
    switch (typeof value) {
      ...
      case 'string':
        if (match = /^\s*(.+?)\s*?(K|M|G|T|E|Z|Y)?(B|b)\s*$/.exec(value)) {
          if ((options.bit != null) && (options.to != null)) {
          ...
          if (options.to) {
            ...
            for (power = _j = 0, _len1 = _ref1.length; _j < _len1; power = ++_j) {
              name = _ref1[power];
              if (options.to === ("" + name + "B")) {
                value = value / Math.pow(1024, power);
              } else if (options.to === ("" + name + "b")) {
